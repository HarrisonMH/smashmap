...
class SquadContextMenu(tk.Frame):
    def __init__(self, master, squad, canvas_item_id, **kwargs):
        super().__init__(master, kwargs)

        self._squad = squad
        self._canvas_item_id = canvas_item_id
        # self._destroy_menu_callback = destroy_menu_callback
        self._create_widgets()

    def _create_widgets(self):
        button_width = 8
        if self._squad.get_location().get_owner() is None:
            self._build_btn = tk.Button(self, text="Build", width=button_width, command=self._context_colonize)
            self._build_btn.pack()
        self._pass_btn = tk.Button(self, text="Pass", width=button_width, command=self._context_pass)
        self._pass_btn.pack()
        self._cancel_btn = tk.Button(self, text="Cancel", width=button_width, command=self.destroy)
